...
@section('action-content')
  ...
  <div class="box-body">
    ...
    <div id="example2_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
                ...
                <tr role="row" class="odd">
                  ...
                  <td>
                    <form class="row" method="POST" action="{{ route('user-management.destroy', ['id' => $user->id]) }}" onsubmit = "return confirm('Are you sure?')">
                        ...
                        <a href="{{ route('user-management.edit', ['id' => $user->id]) }}" title="Edit User" class="btn btn-info col-sm-5 col-xs-6 btn-margin">
                          <i class="fa fa-edit fa-lg"></i>
                        </a>
                        {{-- <a href="{{ route('user-management.edit', ['id' => $user->id]) }}" title="Print User" class="btn btn-primary col-sm-3 col-xs-5 btn-margin">
                          <i class="fa fa-print fa-lg"></i>
                        </a> --}}
                        @if ($user->username != Auth::user()->username)
                         <button type="submit" title="Delete User" class="btn btn-danger col-sm-5 col-xs-6 btn-margin">
      ...
      <div class="row">
        <div class="col-sm-5">
          {{-- <div class="dataTables_info" id="example2_info" role="status" aria-live="polite">{{count($users)}} of {{count($users)}} entries</div> --}}
        </div>
        <div class="col-sm-7">
